...
\newcommand{\source}[1]{\begin{textblock*}{4cm}(11.5cm,8.1cm)
...
\begin{frame}<-9>[label=assets]{Assets}
  ...
  \begin{overlayarea}{\textwidth}{0.3\textheight}
    ...
    \onslide<6-9>{
      \vspace{-1.8em}
      \begin{center}
        \LARGE $\onslide<6-10>{\texttt{0x00f8}} \onslide<7-9>{\times \texttt{0x00c9}}
        \onslide<8-9>{ = \texttt{0xc2b8} } \onslide<9>{ \ll \texttt{0x298e}}$
      \end{center}
    }
    \only<10-11>{
      \begin{figure}
...
\begin{frame}{Assets}
  \begin{figure}
    \only<1>{\includegraphics[scale=1]{172.png}}
    \only<2>{\includegraphics[height=0.2\textheight]{flagtop}}
  \end{figure}
\end{frame}
...
\begin{frame}{Pseudocode}\pause
  ...
  \item \texttt{``When movie 202 finishes, play asset 120.''}
  \end{enumerate}
\end{frame}
